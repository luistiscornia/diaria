   ...
   public function sort($column)
{
    ...
    public function mount()
    {
        ...
        $this->fecha_form = session('ultima-fecha', now()->format('d-m-Y')) ;
    ...
    FROM vw_horario_actual_laravel 
    LEFT JOIN (SELECT id as lugar__id, nombre as lugar__nombre FROM lugar) lugar 
        ON vw_horario_actual_laravel.puesto = lugar.lugar__id 
    WHERE (dia = $this->dia OR 0 = $this->dia AND fecha='".$this->fecha_form."') 
        AND ((guardia_temporal='".$this->guardia_temporal."' OR guardia='".$this->guardia_temporal."') 
        $andWhereEtiqueta 
    ...
    ORDER BY nombre_agente, orden_principal DESC, ingreso ASC
");
...
$data = new Paginator($data, 1000);
    ...
    {
        $this->detalleAgente = DB::table("historial")->where('id_planilla', '=', $this->planillaID)->get();
    }
   ...
   public function actualizarHistorial()
{
    ...
    $fechaStr = '2 de march de 2024'; // Ajusta esta línea según cómo obtengas la fecha
    ...
    public function exportarPlanilla($type)
    {
        ...
        $detalleAgente = DB::table('historial')->whereIn('id_planilla', $cabeceras_ids);

        dd($detalleAgente->toSql());
        // Llama a mostrarHistorial() con el argumento $fechaStr
        // $detalleAgente = DB::table("historial")->where('id_planilla', '=', $this->planillaID)->get();
        if($type == 'excel'){
