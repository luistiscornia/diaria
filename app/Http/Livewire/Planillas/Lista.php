   ...
   public function actualizarHistorial()
{
    ...
    $fechaStr = $fecha->locale('es')->format('d-m-Y'); // Ajusta esta línea según cómo obtengas la fecha
    ...
    public function exportarPlanilla($type)
    {
        ...
        $name = "Planilla " . $fecha->isoFormat('DD-MM-YYYY');
        $fechaStr = $fecha->locale('es')->isoFormat('D [de] MMMM [de] YYYY');
        
        // Obtener plantillas horios det
        $cabeceras = $this->resultData()->items();
        $cabeceras_ids = [];

        foreach ($cabeceras as $cabe) {
            array_push($cabeceras_ids, $cabe->id_planilla_horarios_det);
        }        

        $historial = DB::table('historial_2')->whereIn('id_planilla', $cabeceras_ids)->get();
